...
#Running the Example on Apache Karaf
...
```
5.  Install the ECF gRPC Distribution Provider type
```console
karaf@root()> feature:install -v ecf-rs-distribution-grpc
```

This should result in an install of all of the necessary gRPC Remote Service Provider plugins to support running either/both the HealthCheck example impl or the remote service consumer.
